...
public class CompetenciaConCompresion {
    ...
    static final int MODE_CHANNEL_FILTER = 5;
    static final int MODE_GREEN_CHANNEL_FILTER = 6;
    static final int MODE_RED_CHANNEL_FILTER = 7;
    static final int MODE_BLUE_CHANNEL_FILTER = 8;
    ...
    static void compress(FastScanner fs) throws Exception {
        ...
        byte[] channelFiltered = makeChannelFiltered(raw, n, m);
        byte[] compressedChannelFiltered = deflate(channelFiltered, level, true);
        best = chooseBest(best, new Candidate(MODE_CHANNEL_FILTER, compressedChannelFiltered));

        byte[] greenTransformed = makeGreenTransform(raw, totalPixels);
        byte[] greenChannelFiltered = makeChannelFiltered(greenTransformed, n, m);
        byte[] compressedGreenChannelFiltered = deflate(greenChannelFiltered, level, true);
        best = chooseBest(best, new Candidate(MODE_GREEN_CHANNEL_FILTER, compressedGreenChannelFiltered));

        byte[] redTransformed = makeRedTransform(raw, totalPixels);
        byte[] redChannelFiltered = makeChannelFiltered(redTransformed, n, m);
        byte[] compressedRedChannelFiltered = deflate(redChannelFiltered, level, true);
        best = chooseBest(best, new Candidate(MODE_RED_CHANNEL_FILTER, compressedRedChannelFiltered));

        byte[] blueTransformed = makeBlueTransform(raw, totalPixels);
        byte[] blueChannelFiltered = makeChannelFiltered(blueTransformed, n, m);
        byte[] compressedBlueChannelFiltered = deflate(blueChannelFiltered, level, true);
        best = chooseBest(best, new Candidate(MODE_BLUE_CHANNEL_FILTER, compressedBlueChannelFiltered));
    ...
    static byte[] makeChannelFiltered(byte[] raw, int n, int m) {
        int totalPixels = n * m;

        byte[] result = new byte[4 * n + raw.length];
        ...
        for (int channel = 0; channel < 4; channel++) {
            for (int row = 0; row < n; row++) {
                ...
                for (int col = 0; col < m; col++) {
                    ...
                    int position = pixelIndex * 4 + channel;

                    int current = raw[position] & 255;

                    int left = col > 0 ? raw[position - 4] & 255 : 0;
                    int up = row > 0 ? raw[position - m * 4] & 255 : 0;
                    int upLeft = row > 0 && col > 0 ? raw[position - m * 4 - 4] & 255 : 0;
                ...
                result[channel * n + row] = (byte) bestFilter;

                int outputBase = 4 * n + channel * totalPixels + row * m;
                ...
                for (int col = 0; col < m; col++) {
                    ...
                    result[outputBase + col] = (byte) ((current - predicted) & 255);
                }
            }
        }
    ...
    static byte[] undoChannelFiltered(byte[] data, int n, int m) {
        int totalPixels = n * m;
        ...
        for (int channel = 0; channel < 4; channel++) {
            for (int row = 0; row < n; row++) {
                ...
                int filter = data[channel * n + row] & 255;
                int inputBase = 4 * n + channel * totalPixels + row * m;

                for (int col = 0; col < m; col++) {
                    int pixelIndex = row * m + col;
                    int position = pixelIndex * 4 + channel;

                    int value = data[inputBase + col] & 255;
    ...
    static byte[] makeGreenTransform(byte[] raw, int totalPixels) {
        byte[] result = new byte[raw.length];

        for (int i = 0; i < totalPixels; i++) {
            int position = i * 4;

            int r = raw[position] & 255;
            int g = raw[position + 1] & 255;
            int b = raw[position + 2] & 255;
            int a = raw[position + 3] & 255;

            result[position] = (byte) g;
            result[position + 1] = (byte) ((r - g) & 255);
            result[position + 2] = (byte) ((b - g) & 255);
            result[position + 3] = (byte) a;
        }

        return result;
    }

    static byte[] undoGreenTransform(byte[] transformed, int totalPixels) {
        byte[] raw = new byte[transformed.length];

        for (int i = 0; i < totalPixels; i++) {
            int position = i * 4;

            int g = transformed[position] & 255;
            int differenceR = transformed[position + 1] & 255;
            int differenceB = transformed[position + 2] & 255;
            int a = transformed[position + 3] & 255;

            int r = (g + differenceR) & 255;
            int b = (g + differenceB) & 255;

            raw[position] = (byte) r;
            raw[position + 1] = (byte) g;
            raw[position + 2] = (byte) b;
            raw[position + 3] = (byte) a;
        }

        return raw;
    }

    static byte[] makeRedTransform(byte[] raw, int totalPixels) {
        byte[] result = new byte[raw.length];
        ...
        for (int i = 0; i < totalPixels; i++) {
            ...
            result[position + 1] = (byte) ((g - r) & 255);
            result[position + 2] = (byte) ((b - r) & 255);
            result[position + 3] = (byte) a;
        }
    ...
    static byte[] undoRedTransform(byte[] transformed, int totalPixels) {
        byte[] raw = new byte[transformed.length];

        for (int i = 0; i < totalPixels; i++) {
            int position = i * 4;

            int r = transformed[position] & 255;
            int differenceG = transformed[position + 1] & 255;
            int differenceB = transformed[position + 2] & 255;
            int a = transformed[position + 3] & 255;

            int g = (r + differenceG) & 255;
            int b = (r + differenceB) & 255;
    ...
    static byte[] makeBlueTransform(byte[] raw, int totalPixels) {
        byte[] result = new byte[raw.length];
        ...
        for (int i = 0; i < totalPixels; i++) {
            ...
            result[position + 1] = (byte) ((r - b) & 255);
            result[position + 2] = (byte) ((g - b) & 255);
            result[position + 3] = (byte) a;
        }
    ...
    static byte[] undoBlueTransform(byte[] transformed, int totalPixels) {
        byte[] raw = new byte[transformed.length];

        for (int i = 0; i < totalPixels; i++) {
            int position = i * 4;

            int b = transformed[position] & 255;
            int differenceR = transformed[position + 1] & 255;
            int differenceG = transformed[position + 2] & 255;
            int a = transformed[position + 3] & 255;

            int r = (b + differenceR) & 255;
            int g = (b + differenceG) & 255;
    ...
    static void decompress(FastScanner fs) throws Exception {
        ...
        if (k == totalPixels) {
            ...
            } else if (mode == MODE_CHANNEL_FILTER) {
                byte[] raw = undoChannelFiltered(data, n, m);
                pixels = bytesToPixels(raw, totalPixels);
            } else if (mode == MODE_GREEN_CHANNEL_FILTER) {
                byte[] greenChannelData = undoChannelFiltered(data, n, m);
                byte[] raw = undoGreenTransform(greenChannelData, totalPixels);
                pixels = bytesToPixels(raw, totalPixels);
            } else if (mode == MODE_RED_CHANNEL_FILTER) {
                byte[] redChannelData = undoChannelFiltered(data, n, m);
                byte[] raw = undoRedTransform(redChannelData, totalPixels);
                pixels = bytesToPixels(raw, totalPixels);
            } else if (mode == MODE_BLUE_CHANNEL_FILTER) {
                byte[] blueChannelData = undoChannelFiltered(data, n, m);
                byte[] raw = undoBlueTransform(blueChannelData, totalPixels);
                pixels = bytesToPixels(raw, totalPixels);
